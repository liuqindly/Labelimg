...
class Canvas(QtWidgets.QWidget):
        ...
        if self.drawing():
            ...
            if self.outOfPixmap(pos):
                ...
                pos = self.intersectionPoint(self.polcurrent[-1], pos)
                    ...
                    self.closeEnough(pos, self.polcurrent[0]):
    ...
    def canCloseShape(self):
        return self.drawing() and self.polcurrent and len(self.polcurrent) > 2

    def mouseDoubleClickEvent(self, ev):
        # We need at least 4 points here, since the mousePress handler
        # adds an extra one before this handler is called.
        if self.canCloseShape() and len(self.polcurrent) > 3:
            self.polcurrent.popPoint()
            self.finalise()
    ...
    def paintEvent(self, event):
        ...
        if (self.fillDrawing() and self.createMode == 'polygon' and
                self.polcurrent is not None and len(self.polcurrent.points) >= 2):
            
            drawing_shape = self.polcurrent.copy()
            drawing_shape.addPoint(self.polline[1])
    ...
    def finalise(self):
        ...
        self.shapes.append(self.polcurrent)
        self.storeShapes()
        self.polcurrent = None
        self.setHiding(False)
        self.newShape.emit()
    ...
    def closeEnough(self, p1, p2):
        ...
        return labelme.utils.distance(p1 - p2) < (self.epsilon-6)
    ...
    def keyPressEvent(self, ev):
        key = ev.key()
        if key == QtCore.Qt.Key_Escape and self.polcurrent:
            self.polcurrent = None
    ...
    def undoLastLine(self):
        ...
        self.polcurrent = self.polygons.pop()
        self.polcurrent.setOpen()
        if self.createMode in ['polygon', 'linestrip']:
            self.polline.points = [self.polcurrent[-1], self.polcurrent[0]]
        elif self.createMode in ['rectangle', 'line', 'circle']:
            self.polcurrent.points = self.polcurrent.points[0:1]
        elif self.createMode == 'point':
            self.polcurrent = None
        self.drawingPolygon.emit(True)

    def undoLastPoint(self):
        if not self.polcurrent or self.polcurrent.isClosed():
            return
        self.polcurrent.popPoint()
        if len(self.polcurrent) > 0:
            self.polline[0] = self.polcurrent[-1]
        else:
            self.polcurrent = None
            self.drawingPolygon.emit(False)
